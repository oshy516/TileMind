...
# if not os.path.exists('ilp_params_single.py') or len(sys.argv) > 1:
...
params_file = 'ilp_params_single.py'
if len(sys.argv) > 1 and 'ilp_params' in sys.argv[1] and sys.argv[1].endswith('.py'):
...
def optimize_single_block(block_vars, var_to_size, block_idx, matrix_info, global_valid_tiles):
    ...
    if filtered_ro_constraint is not None:
        # 检查只读约束是否不包含tile_factor变量（即只是常量）
        ...
        # 如果只读约束只包含常量（没有tile变量），且有读写约束，认为所有数组都是读写数组
        if not contains_tile_var and filtered_rw_constraint is not None:
            print("\n=== Detected constant read-only constraint and read-write arrays ===")
            print("Treating all arrays as read-write and forcing them to fit in L1 cache")
            all_read_write = True
    elif filtered_ro_constraint is None and filtered_rw_constraint is not None:
        # 没有只读约束但有读写约束，同样视为所有数组都是读写数组
        print("\n=== Detected all arrays are read-write arrays ===")
        # print("Applying special logic to put all arrays in L1 cache")
        # all_read_write = True

    # 评估只读数组的L1缓存约束
    if filtered_ro_constraint and not all_read_write:
    ...
    if filtered_rw_constraint:
        ...
        # **** 修改逻辑：对于全部是读写数组的情况 ****
        # if all_read_write:
        #     print("All arrays are read-write: forcing arrays to fit in L1 cache")
        ...
        if constraint_valid:
            for combo in all_combinations:
                ...
                try:
                    ...
                    # **** 修改逻辑：对于全部是读写数组的情况 ****
                    if all_read_write:
                        # 全部放入L1缓存，所以限制大于L1_size的组合
                        if constraint_value > L1_size:
                            constraint_name = f"rw_L1_all_" + "_".join([f"{var}_{t}" for var, t in combo])
                            prob += pulp.lpSum(tile_vars[var][t] for var, t in combo) <= len(combo) - 1, constraint_name
                        # 移除L2上限约束的检查，因为我们只关心数据是否能放入L1缓存
                    else:
                        # 正常逻辑
...
if __name__ == "__main__":
    ...
    if has_multiple_blocks:
        ...
        print(f"\nTotal optimization time: {optimization_end - optimization_start:.3f} seconds")
    else:
        print("Using traditional single-block optimization.")
        # 使用原来的单块优化方法
        from ilp_params_single import *
        optimization_start = time.time()
        solution, obj_value = solve_tiling_strategy(matrix_info)
        optimization_end = time.time()
        print(f"\nTotal optimization time: {optimization_end - optimization_start:.3f} seconds")
