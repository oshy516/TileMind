...
def optimize_single_block(block_vars, var_to_size, block_idx, matrix_info, global_valid_tiles):
    ...
    if block_rw_constraint:
        ...
        # **** 新增：全局检查逻辑 ****
        if not skip_l1_lower_constraint and not all_read_write:
            print("\n=== Checking if most combinations violate L1 lower bound ===")
            violated_count = 0
            ...
            violation_ratio = violated_count / total_count if total_count > 0 else 0
            print(f"L1 lower bound violations: {violated_count}/{total_count} combinations ({violation_ratio:.1%})")
            
            # 如果大多数组合都违反L1下界，自动跳过这个约束
            if violation_ratio >= 0.95:  # 95%以上的组合都违反
                print("WARNING: Most combinations violate L1 lower bound!")
