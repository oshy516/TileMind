...
if os.path.exists('ilp_params.py'):
    print("Found existing ilp_params.py file, using pre-generated parameters...")
    print("Skipping preprocessing step to avoid duplication.")
else:
    # 只有在没有参数文件时才运行预处理
    if len(sys.argv) > 1:
        source_file = sys.argv[1]
        print(f"No ilp_params.py found. Running benchmark_preprocess.py on {source_file}...")
        result = subprocess.run(['python3', 'benchmark_preprocess.py', source_file])
        if result.returncode != 0:
            print("Error: benchmark_preprocess.py failed")
            sys.exit(1)
    else:
        print("Error: No ilp_params.py file found and no source file provided")
        print("Usage: python3 benchmark_ilp_tiling_auto.py <source_file>")
        sys.exit(1)
...
def optimize_single_block(block_vars, var_to_size, block_idx, matrix_info, global_valid_tiles):
    ...
    try:
        ...
        if filtered_ro_constraint is not None:
            contains_tile_var = False
            for var in block_vars:
                if f"{var}_tile" in filtered_ro_constraint:
                    contains_tile_var = True
                    break
            
            if not contains_tile_var and filtered_rw_constraint is not None:
                use_l1_optimization = True
                optimization_reason = "Block-level analysis: Detected constant read-only constraint and read-write arrays"
        elif filtered_ro_constraint is None and filtered_rw_constraint is not None:
            use_l1_optimization = True
            optimization_reason = "Block-level analysis: No read-only constraint but has read-write constraint"

    # 评估只读数组的L1缓存约束
    if filtered_ro_constraint and not use_l1_optimization  :
    ...
    if filtered_rw_constraint:
        ...
        if filtered_ro_constraint and not skip_l1_lower_constraint:
            ...
            if len(all_combinations) <= 1000:
                ...
                sample_var_tiles = {}
                for var in block_vars:
                    if var in var_to_active_tiles:
                        tiles = var_to_active_tiles[var]
                        # 取前几个、中间几个、后几个
            ...
            for combo in check_combinations:
                ...
                try:
                    locals().update(eval_vars)
                    rw_value = eval(filtered_rw_constraint)
                    ro_value = eval(filtered_ro_constraint)
        ...
        if not skip_l1_lower_constraint:
            ...
            if violated_count >= total_count * 1:  # 95%以上的组合都违反
        ...
        try:
            ...
            for var, t in combo:
                if var in column_major_dims :
                    if var in block_var_to_size:  # Add this check
                        dim_size = block_var_to_size[var]
    ...
    prob.solve(pulp.PULP_CBC_CMD(msg=False, timeLimit=600))
    solve_end = time.time()
    print(f"Solve time: {solve_end - solve_start:.2f} seconds")
        ...
        for var, tile in selected_tiles.items():
            ...
            if var in column_major_dims:
                if var in block_var_to_size:  # Add this check
                    ...
                    # Handle the case where the dimension size is not in the dictionary
                    print(f"  {var} is a column-major dimension, but dimension size not found in block_var_to_size")
...
if __name__ == "__main__":
    ...
    if has_multiple_blocks:
        ...
        print(f"\nTotal ILP solving time: {optimization_end - optimization_start:.3f} seconds")
    else:
