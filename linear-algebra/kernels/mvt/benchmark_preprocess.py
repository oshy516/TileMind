...
def get_tileable_dimensions(source_file):
    """
    Run detect_dimension_mp.py to get information about which dimensions are tileable
    ...
    try:
        # 修正：对于所有类型的处理后的文件，都回溯到原始源文件
        original_source_file = None
        
        # 处理各种可能的文件名模式
        if "_block" in source_file and ("_double.c" in source_file or "_float.c" in source_file):
            # 处理块文件：gemm_mp_ppcg_default_block0_double.c -> gemm.c
            # 提取基础文件名（gemm）
            base_name = source_file.split('_')[0]  # 获取 "gemm"
            original_source_file = f"{base_name}.c"
            print(f"Detected block file. Using original source file: {original_source_file}")
            
        elif source_file.endswith("_mp_ppcg_default.c"):
            # 处理PPCG输出文件：gemm_mp_ppcg_default.c -> gemm.c
            original_source_file = source_file.replace("_mp_ppcg_default.c", ".c")
            print(f"Detected PPCG output file. Using original source file: {original_source_file}")
            
        elif source_file.endswith("_ppcg_default.c"):
            # 处理标准PPCG输出文件：gemm_ppcg_default.c -> gemm.c
            original_source_file = source_file.replace("_ppcg_default.c", ".c")
            print(f"Detected standard PPCG output file. Using original source file: {original_source_file}")
        else:
            # 如果是原始文件，直接使用
            original_source_file = source_file
            print(f"Using source file as-is: {original_source_file}")
        
        # 验证原始源文件是否存在
        if not os.path.exists(original_source_file):
            print(f"Warning: Original source file {original_source_file} not found")
            # 尝试在当前目录查找
            possible_files = [f for f in os.listdir('.') if f.endswith('.c') and not ('_' in f or 'ppcg' in f)]
            if possible_files:
                original_source_file = possible_files[0]
                print(f"Using fallback source file: {original_source_file}")
            else:
                print("No suitable source file found")
                return {}
        ...
        # 检查源文件名来决定使用哪个脚本
        if "mp" in source_file:
            detect_script = os.path.join(base_dir, "dimension_detect_mp.py")
            print(f"Using MP version dimension detection script: {detect_script}")
        else:
            detect_script = os.path.join(base_dir, "dimension_detect.py")
            print(f"Using standard dimension detection script: {detect_script}")
        
        # 检查脚本是否存在
        if not os.path.exists(detect_script):
            print(f"Warning: {os.path.basename(detect_script)} not found at {detect_script}")
            
            # 尝试备用脚本
            if "mp" in source_file:
                backup_script = os.path.join(base_dir, "dimension_detect.py")
                if os.path.exists(backup_script):
                    detect_script = backup_script
                    print(f"Using backup script: {detect_script}")
                else:
                    print("No dimension detection script found")
                    return {}
            else:
                backup_script = os.path.join(base_dir, "dimension_detect_mp.py")
                if os.path.exists(backup_script):
                    detect_script = backup_script
                    print(f"Using backup MP script: {detect_script}")
                else:
                    print("No dimension detection script found")
                    return {}
        
        # 执行维度检测命令 - 关键修正：始终使用原始源文件
        cmd = [sys.executable, detect_script, original_source_file]
        ...
        if process.returncode != 0:
            print(f"Error running dimension detection script: {stderr}")
            return {}
...
def map_filters_to_blocks(tileable_dimensions, loop_info):
    """
    Map filters from detect_dimension_mp.py to blocks from loop analysis
...
def analyze_array_accesses(code_blocks, latencies, data_type, loop_info):
    ...
    # 第一阶段：预扫描所有语句，收集所有被访问的数组（读取和写入）
    all_arrays_written = set()
    all_arrays_accessed = set()  # 新增：所有被访问的数组
    
    # 从loop_info的语句中提取被写入的数组和所有被访问的数组
    for stmt in loop_info_statements:
        ...
        if lhs_match:
            ...
            all_arrays_accessed.add(array_name)
        
        # 提取右侧的数组名称（读取）
        rhs_arrays = re.findall(r'(\w+)(?:\[[^\]]+\])+', stmt)
        for array_name in rhs_arrays:
            all_arrays_accessed.add(array_name)
    
    # 区分只读数组和读写数组
    read_only_arrays = all_arrays_accessed - all_arrays_written
    read_write_arrays = all_arrays_written
    
    print(f"\nPre-scan phase analysis:")
    print(f"  All arrays accessed: {all_arrays_accessed}")
    print(f"  Read-write arrays (will use L2 cache): {read_write_arrays}")
    print(f"  Read-only arrays (will use L1 cache): {read_only_arrays}")
    
    # 修正后的缓存策略判断逻辑：
    # 1. 如果整个循环块只访问一个数组（无论读写），使用L1缓存优化
    # 2. 如果有多个数组：
    #    - 只读数组使用L1缓存
    #    - 读写数组使用L2缓存
    use_single_array_optimization = len(all_arrays_accessed) == 1
    
    if use_single_array_optimization:
        single_array = next(iter(all_arrays_accessed))
        print(f"Single array optimization: Only one array ({single_array}) accessed in entire computation. Using L1 cache for all accesses.")
    else:
        print(f"Multiple arrays detected ({len(all_arrays_accessed)} arrays). Using differentiated cache allocation:")
        print(f"  - Read-only arrays ({len(read_only_arrays)}): L1 cache")
        print(f"  - Read-write arrays ({len(read_write_arrays)}): L2 cache")
    ...
    access_info = {
        ...
        'read_only_arrays': read_only_arrays.copy(),
        'read_write_arrays': read_write_arrays.copy(),
        'blocks_info': [],
    ...
    for block in blocks:
        ...
        for idx, (lhs, op, rhs) in enumerate(statements_to_process):
            ...
            print(f"  Processing cache access for stmt {idx} in block {block_idx}")
            ...
            for array in stmt_info['arrays_read']:
                ...
                elif array in read_write_arrays:
                    # 读写数组使用L2缓存
                    l2_count += read_count
                    print(f"  Array {array} read({read_count}) in stmt {idx} of block {block_idx}: {read_count}*L2 access (read-write array)")
                else:
                    # 只读数组使用L1缓存
                    l1_count += read_count
            ...
            for array in stmt_info['arrays_written']:
                ...
                if array not in stmt_info['arrays_read']:
                    if use_single_array_optimization:
                        ...
                        print(f"  Array {array} write-only({write_count}) in stmt {idx} of block {block_idx}: {write_count}*L2 access (read-write array)")
                    else:
                        # 这种情况理论上不应该发生，因为写入的数组应该都在read_write_arrays中
                        print(f"  Warning: Array {array} is written but not in read_write_arrays")
                        l2_count += write_count
            ...
            stmt_info['cost'] = (
                ...
                math_cost  # NEW: Include math function costs
            )
        ...
        for array, counts in block_access_info['arrays'].items():
            ...
            if use_single_array_optimization:
                ...
                cache_type = "L2 (read-write array)"
            else:
                cache_type = "L1 (read-only array)"
                
            print(f"Array {array} in block {block_idx}: {reads} reads, {writes} writes, cache type: {cache_type}")
