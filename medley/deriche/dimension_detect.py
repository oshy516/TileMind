...
def parse_schedule_tree(schedule_output):
    ...
    # 修复正则表达式以仅匹配顶级filter块（通过检查缩进级别）
    # 在schedule树中，顶级filter块通常有一个固定的缩进级别（2个空格）
    top_level_filter_lines = []
    ...
    for i, line in enumerate(lines):
        ...
        if re.match(r'^\s{2}- filter:', line):
            ...
            if filter_match:
                ...
                top_level_filter_lines.append((filter_text, child_text))
    
    if not top_level_filter_lines:
        print("No top-level filter blocks found")
        return dimensions
    
    print(f"\nFound {len(top_level_filter_lines)} top-level filter blocks")
    
    # 分析每个顶级filter块
    for idx, (filter_text, child_text) in enumerate(top_level_filter_lines):
