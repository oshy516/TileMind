...
def get_tileable_dimensions(source_file):
    ...
    Consider both "permutable AND coincident" dimensions as well as "permutable only" dimensions
    within OpenMP parallel regions as tileable
    
    Args:
        source_file: Path to the source file to analyze
        
    Returns:
        Dictionary mapping filter IDs to lists of tileable dimensions and set of OpenMP parallelizable filter IDs
    """
    print("\nDetecting tileable dimensions...")
    try:
        # Run detect_dimension.py script on the same source file
        detect_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "dimension_detect_openmp.py")
        if not os.path.exists(detect_script):
            print(f"Warning: detect_dimension_openmp.py not found at {detect_script}")
            return {}
        ...
        if process.returncode != 0:
            print(f"Error running detect_dimension_openmp.py: {stderr}")
            return {}
        
        # Parse the output to extract tileable dimensions
        tileable_dimensions = {}
        openmp_parallelizable_filters = set()
        
        # Flags to track which section we're in
        in_tileable_section = False
        in_permutable_only_section = False
        in_openmp_section = False
        
        # Process each line of the output
        for line in stdout.split('\n'):
            # Check if we're entering the tileable section
            if "Tileable dimensions" in line:
                in_tileable_section = True
                in_permutable_only_section = False
                in_openmp_section = False
                continue
            
            # Check if we're entering the permutable-only section
            if "Non-tileable dimensions" in line or "permutable but not coincident" in line:
                in_tileable_section = False
                in_permutable_only_section = True
                in_openmp_section = False
                continue
            
            # Check if we're entering the OpenMP section
            if "OpenMP parallelizeable:" in line:
                in_tileable_section = False
                in_permutable_only_section = False
                in_openmp_section = True
                continue
            
            # Process OpenMP parallelizable filters
            if in_openmp_section:
                filter_match = re.search(r'-\s*Filter\s+(\d+)', line)
                if filter_match:
                    filter_id = int(filter_match.group(1))
                    openmp_parallelizable_filters.add(filter_id)
                    print(f"Detected OpenMP parallelizable filter: {filter_id}")
                    continue
            ...
            if match:
                ...
                # Process dimensions based on section
                if in_tileable_section:
                    # Always consider permutable AND coincident dimensions as tileable
                ...
                elif in_permutable_only_section:
                    if filter_id in openmp_parallelizable_filters:
                        # If filter is OpenMP parallelizable, consider permutable-only dimensions as tileable
                        tileable_dimensions[filter_id].append((dimension, var_name))
                        print(f"Found tileable dimension (permutable in OpenMP region): Filter {filter_id}, Dimension {dimension}: ({var_name})")
                    else:
                        print(f"Found non-tileable dimension: Filter {filter_id}, Dimension {dimension}: ({var_name})")
        ...
        print(f"Total tileable filters: {len(tileable_dimensions)}")
        for filter_id, dimensions in tileable_dimensions.items():
            print(f"Filter {filter_id} has {len(dimensions)} tileable dimensions: {dimensions}")
        
        # Print OpenMP parallelizable filters
        if openmp_parallelizable_filters:
            print(f"\nOpenMP parallelizable filters: {openmp_parallelizable_filters}")
        else:
            print("\nNo OpenMP parallelizable filters detected")
        
        return tileable_dimensions, openmp_parallelizable_filters
...
def analyze_loop_structure(code_blocks):
    ...
    for block_idx, code_block in enumerate(code_blocks):
        ...
        for i, match in enumerate(loops_found):
            ...
            # 改进的嵌套级别检测 - 不区分 OpenMP 与非 OpenMP 模式
            # 使用统一的方法来处理所有循环
            for prev_match in loops_found[:i]:
                ...
                # 跳过重复的变量名检查 - 修复同名变量bug
                if prev_var == var and prev_unique != unique_var:
                    # 检查是否真的是不同的循环而不是相同循环的多次引用
                    prev_line = code_block[:prev_start].count('\n')
                    curr_line = code_block[:loop_start].count('\n')
                    if prev_line != curr_line:  # 如果在不同行，可能是不同的循环
                        # 检查变量范围，避免将重用的相同变量名错误地视为嵌套关系
                        continue
                    ...
                    if body_text[pos] == '{':
                        ...
                        for p in range(pos + 1, len(body_text)):
                            if body_text[p] == '{':
                                ...
                                open_braces -= 1
                                if open_braces == 0:
                                    close_pos = prev_start + p + 1  # +1包括结束大括号
                                    break
                    else:
                        # 处理没有大括号的循环体或格式不标准的情况
                        # 找到语句结束的分号
                        for p in range(pos, len(body_text)):
                            if body_text[p] == ';':
                                close_pos = prev_start + p + 1  # +1包括分号
                                break
                
                # 如果找到了完整的循环范围
                if close_pos != -1:
                    # 检查当前循环是否真的在前一个循环内
                    if prev_start < loop_start < close_pos:
                        prev_level = loop_levels.get(prev_unique, 0)
                        level = max(level, prev_level + 1)
                        print(f"Loop {unique_var} is nested inside {prev_unique}, level set to {level}")
                        
                        # 检查这是否是一个有 OpenMP 指令的循环
                        # 但 OpenMP 指令不影响嵌套结构的判断
                        omp_text = code_block[prev_start:loop_start] 
                        # 在检测到OpenMP指令的代码部分添加以下内容
                        if "#pragma omp" in omp_text:
                            print(f"  OpenMP directive detected before {unique_var} but maintaining proper nesting level {level}")
                            # 新增：标记该块为OpenMP并行块，并设置该维度为并行主要维度
                            if 'openmp_parallel_blocks' not in loop_info:
                                loop_info['openmp_parallel_blocks'] = {}
                            loop_info['openmp_parallel_blocks'][block_idx] = True
                            
                            # 新增：记录并行的主要维度
                            if 'openmp_parallel_dims' not in loop_info:
                                loop_info['openmp_parallel_dims'] = {}
                            if block_idx not in loop_info['openmp_parallel_dims']:
                                loop_info['openmp_parallel_dims'][block_idx] = []
                            if unique_var not in loop_info['openmp_parallel_dims'][block_idx]:
                                loop_info['openmp_parallel_dims'][block_idx].append(unique_var)
                                print(f"  Marking {unique_var} as parallel dimension for block {block_idx}")
        ...
        # Find all matches with the improved pattern
...
def analyze_array_accesses(code_blocks, latencies, data_type, loop_info):
    ...
    # 新增：判断是否只有一个数组，并根据结果决定缓存策略
    use_single_array_optimization = len(all_arrays_written) == 1
...
def generate_objective_function(loop_info, access_info, matrix_info):
    ...
    for stmt in statements:
        ...
        # Get loop variables for this block in order of nesting level
        block_vars = block_to_vars.get(block_idx, [])
        loop_vars_by_level = {}
        
        # Organize loop variables by their nesting level
        for var in block_vars:
        ...
        # Determine the loops this statement is nested within
        stmt_nesting_level = -1
        
        # First find the statement's own nesting level based on variables it uses
        stmt_loop_vars = stmt.get('loop_vars', [])
        for var in stmt_loop_vars:
            if var in loop_levels:
                stmt_nesting_level = max(stmt_nesting_level, loop_levels[var])
        
        # Collect all loops that contain this statement based on nesting structure
        enclosing_loops = []
        for level in sorted(loop_vars_by_level.keys()):
            # If this level is an ancestor of the statement's nesting level
            # or the statement uses variables at this level, include it
            if level <= stmt_nesting_level or any(var in stmt_loop_vars for var in loop_vars_by_level[level]):
                enclosing_loops.extend(loop_vars_by_level[level])
        
        # If no loops found, use all loops in the block (this is safer)
        if not enclosing_loops:
            enclosing_loops = block_vars
...
def write_parameters_to_file(params, output_file):
    ...
    with open(output_file, 'w') as f:
        ...
        # Write OpenMP parallelizable blocks information
        if 'openmp_parallel_blocks' in params:
            f.write("# OpenMP parallelizable blocks\n")
            parallel_blocks = [idx for idx, is_parallel in params['openmp_parallel_blocks'].items() if is_parallel]
            if parallel_blocks:
                # Format as comments to be more readable
                f.write("# ")
                if len(parallel_blocks) == 1:
                    f.write(f"Block {parallel_blocks[0]}\n")
                    # 新增：输出并行的主要维度
                    if 'openmp_parallel_dims' in params and parallel_blocks[0] in params['openmp_parallel_dims']:
                        parallel_dims = params['openmp_parallel_dims'][parallel_blocks[0]]
                        if parallel_dims:
                            f.write(f"# (Parallel dimensions: {', '.join(parallel_dims)})")
                    f.write("\n")
                else:
                    for idx in sorted(parallel_blocks):
                        f.write(f"Block {idx}")
                        # 新增：输出并行的主要维度
                        if 'openmp_parallel_dims' in params and idx in params['openmp_parallel_dims']:
                            parallel_dims = params['openmp_parallel_dims'][idx]
                            if parallel_dims:
                                f.write(f"# (Parallel dimensions: {', '.join(parallel_dims)})")
                        f.write("\n# ")
                f.write("\n")
            ...
            # Now write the actual Python dictionary
            f.write("openmp_parallel_blocks = {\n")
            for block_idx, is_parallel in sorted(params['openmp_parallel_blocks'].items()):
                f.write(f"    {block_idx}: {is_parallel},\n")
            f.write("}\n\n")

            # 新增：输出并行维度的Python字典
            if 'openmp_parallel_dims' in params:
                f.write("# OpenMP parallel dimensions\n")
                f.write("openmp_parallel_dims = {\n")
                for block_idx, dims in sorted(params['openmp_parallel_dims'].items()):
                    if dims:  # 只输出有维度的块
...
def main():
    ...
    if code_blocks:
        ...
        # 获取可分块维度信息
        tileable_dimensions, openmp_parallelizable_filters = get_tileable_dimensions(args.source_file)
        block_tileable_dims = map_filters_to_blocks(tileable_dimensions, loop_info)
        
        # Update openmp_parallel_blocks based on the filter information
        openmp_parallel_blocks = {}
        for filter_id in openmp_parallelizable_filters:
            # Convert to 0-based index if filters are 1-based
            block_idx = filter_id - 1 if filter_id > 0 else filter_id
            openmp_parallel_blocks[block_idx] = True
        
        # Merge with existing OpenMP information from loop structure analysis
        if 'openmp_parallel_blocks' in loop_info:
            # If a block is marked as parallelizable in either source, keep it
            for block_idx, is_parallel in loop_info['openmp_parallel_blocks'].items():
                if is_parallel:
                    openmp_parallel_blocks[block_idx] = True
        
        # Update the loop_info structure
        loop_info['openmp_parallel_blocks'] = openmp_parallel_blocks
        loop_info['block_tileable_dims'] = block_tileable_dims
        
        # Print OpenMP parallelizable blocks
        print("\nOpenMP parallelizable blocks:")
        parallel_blocks = [idx for idx, is_parallel in openmp_parallel_blocks.items() if is_parallel]
        if 'openmp_parallel_blocks' in loop_info and any(loop_info['openmp_parallel_blocks'].values()):
            parallel_blocks = [idx for idx, is_parallel in loop_info['openmp_parallel_blocks'].items() if is_parallel]
            for block_idx in sorted(parallel_blocks):
                print(f"Block {block_idx}")
                # 新增：输出并行的主要维度
                if 'openmp_parallel_dims' in loop_info and block_idx in loop_info['openmp_parallel_dims']:
                    parallel_dims = loop_info['openmp_parallel_dims'][block_idx]
                    if parallel_dims:
                        print(f"  Parallel dimensions: {', '.join(parallel_dims)}")
        else:
            print("None identified")
            
        # Add information to all_params
        all_params['block_tileable_dims'] = block_tileable_dims
        all_params['openmp_parallel_blocks'] = openmp_parallel_blocks

        if 'openmp_parallel_dims' in loop_info:
            all_params['openmp_parallel_dims'] = loop_info['openmp_parallel_dims']
